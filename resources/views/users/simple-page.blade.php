...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#8a8a8a] flex items-center lg:justify-center min-h-screen flex-col">
    <div class="w-[90%] flex row min-h-300 justify-around">
        <div class="shrink-0 w-[50%]">
            ...
            <div id="controls" class="flex justify-between mt-5">
                <div class="flex row">
                    <div id="prev" class="control"><span>Prev</span></div>
                    <div id="next" class="control"><span>Next</span></div>
                </div>
                <div class="flex row items-center">
                    ...
                    <div class="option">
                        <label>Limit<input id="limit" type="number"></label>
                    </div>
                    <div id="apply" class="control"><span>Apply</span></div>
                </div>
            </div>
        </div>
            ...
            <div class="mt-5">
                <div class="flex justify-between items-center text-sm font-medium mb-1">
                    <label class="flex row items-center">
                        <p>
                            <span id="image-count">1</span> / 15 Images *
                        </p>
                    </label>
                    <button
                        type="button"
                ...
                <div id="images-container" class="flex row flex-wrap">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('form').addEventListener('submit', async (e) => {
                ...
                const data = collectData(new FormData(e.target));
        ...
        function refreshControls(meta) {
            document.getElementById('limit').value = limit;
            document.getElementById('offset').value = offset;

            let next = document.getElementById('next');
            if (!meta.has_next_page) {
                next.classList.add('inactive-control');
                next.classList.add('disabled');
            } else {
                next.classList.remove('inactive-control');
                next.classList.remove('disabled');
            }

            let prev = document.getElementById('prev');
            if (offset === 0) {
                prev.classList.add('inactive-control');
                prev.classList.add('disabled');
            } else {
                prev.classList.remove('inactive-control');
        ...
        async function refreshPage() {
                ...
                <table class="w-full border border-gray-300">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th class="border border-gray-300 px-4 py-2 text-left">Images count</th>
                        </tr>
                    </thead>
                    <tbody>
            `;
            ...
            data.data.forEach(user => {
                tableHTML += `
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border border-gray-300 px-4 py-2">${user.images_count}</td>
                    </tr>
                `;
            });
            ...
            refreshControls(data.meta)
        }

        function displayPage(next = true) {
            offset = next ? offset + limit : Math.max(offset - limit, 0);
        ...
        function addImageInput() {
            ...
            newImageInput.className = 'image-input-group flex gap-1 items-center mr-5 mt-5 w-[47%]';
            newImageInput.innerHTML = `
                <input
                    type="text"
                    name="images[${imageIndex}][image]"
                ...
                <button
                    type="button"
                    class="remove-image-btn px-3 py-2 bg-red-500 text-white rounded hover:bg-red-600"
                >
                    -
                </button>
            `;

            document.getElementById('images-container').appendChild(newImageInput);
            imageIndex++;
            imageCount++;
            updateAddButton();
            updateRemoveButtons();
            updateImageDisplayCount();
        }

        function removeImageInput(imageGroup) {
            imageGroup.remove();
            imageCount--;
            updateRemoveButtons();
            updateAddButton();
        ...
        function collectData(formData) {
            ...
            imageGroups.forEach((group, index) => {
                const imageInput = group.querySelector('input[type="text"]');
                data.images.push({
